...
import {myBridgesError} from "./utils/Bridges";
import {CustomError} from "./utils/CustomErrors";
import {cloneBuffer, getAppToken, isRequestValid} from "./service/RequestService";
...
const processRequest = async function (
  ...
  try {
    ...
    const responseAction = await ActionService.processRequest(doistRequest, getAppToken(request));
...
const errorResponse = (response: Response, statusCode: number) => {
  response.status(statusCode).json({ bridges: myBridgesError });
};
